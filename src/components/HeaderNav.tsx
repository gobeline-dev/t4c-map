...
import { Map, ShieldCheck, Sun, Moon, Menu, X } from 'lucide-react';
...
const HeaderNav = () => {
  ...
  const isActive = (path: string) =>
    location.pathname === path || (path === '/maps' && location.pathname === '/');
      ...
      <nav className="mx-auto max-w-7xl px-6 lg:px-8">
        <div className="flex h-16 items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-2">
            ...
            >
              {isParchment ? <Moon size={16} className="text-primary-strong" /> : <Sun size={16} className="text-muted-foreground" />}
            </button>
          </div>
        ...
        {isMobileOpen && (
          <div className="md:hidden pb-4 flex flex-col gap-1">
            ...
            >
              {isParchment ? <Moon size={16} /> : <Sun size={16} />}
              {isParchment ? 'Thème Parchemin' : 'Thème Classique'}
            </button>
          </div>
        )}
      </nav>
